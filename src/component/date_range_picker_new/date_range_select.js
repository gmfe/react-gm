...
/** 日期选择 */
// 初始选中日期判断: -- 开始日期 && 结束日期 同月份，日历组件展示 该月份 && 下个月份
//                -- 开始日期 && 结束日期 为null, 日历组件展示 当前月份 && 下个月份
...
const RangeCalendar = props => {
  ...
  const handleSelectDay = (position, date) => {
    onSelectDay(position, date)
  }

  // 判断当前两个日历日期 同年相邻月份 or 不同年, 需展示日历左右 年 / 月 切换按钮
  const isNearMonth = () => {
    const isNearMonth =
