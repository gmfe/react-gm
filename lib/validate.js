...
var ValidateTip = function (rule) {
    ...
    if (noRangeRuleKeys.indexOf(info.type) === -1) {
        if (info.max) {
            return Util.format(ruleKeyTipMap.w[info.type][3], info);
        } else if(info.cross){
            return Util.format(ruleKeyTipMap.w[info.type][2], info);
        } else if (info.min) {
            return Util.format(ruleKeyTipMap.w[info.type][1], info);
        }
