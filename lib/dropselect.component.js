...
class DropSelect extends React.Component {
    ...
    render() {
        ...
        if (loading) {
            coolList =
                <li className="gm-dropselect-item"><span>&nbsp;</span><i className="icon icon-spin ico-spinner2"></i></li>;
        } else {
            coolList = list.map(rowData => {
                const row = columns.map(col => {
                    ...
                    if (col.render) {
                        ...
                        return <div className="gm-ellipsis" style={{flex: '1'}} key={value + field} title={val}>{val}</div>
                    } else {
