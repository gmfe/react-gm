    ...
    render: function () {
        ...
        let optionList = list.map((data, i) => {
            return (<li className={(selectedIndex == i) ? "option-item option-hover option-selected" : "option-item"}
                        ...
                        onClick={this.selectOption.bind(this, data, i)}>{data.name}</li>);
        });
        ...
        return (
            <div className={this.state.opened ? "gm-select gm-open" : "gm-select"}>
                ...
                <input id={this.state.id} ref="input" type="text" className={inputClass} style={inputStyle}
                       value={inputValue}
                       onChange={this.changeInputValue} onKeyUp={this._keyup} onKeyDown={this._keydown}
                       onClick={this._open}></input>
            </div>
        );
    },
